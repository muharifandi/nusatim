...
## Fase 31 — Pengaturan SMTP dari Panel Admin (Site Settings) ✅ (selesai 2026-08-09)

Latar belakang: setelah deploy Fase 30 ke hosting production nusatim.com, ditemukan mail server hosting (`mail.nusatim.com`, relay lewat `smtp.mailchannels.net`) gagal kirim ke Gmail dengan `421 4.3.0 Authentication error` — dikonfirmasi lewat cPanel Track Delivery bahwa ini murni masalah kredensial relay MailChannels di level WHM (bukan bug aplikasi; SPF/DKIM/DMARC domain semua Valid, config Laravel/Symfony Mailer sudah benar per dokumentasi resmi). User cuma punya akses cPanel biasa, bukan WHM/root, jadi tidak bisa memperbaiki ini sendiri di server itu.

Solusi: bukan cuma ganti provider SMTP di `.env` (yang berarti tiap ganti provider harus masuk Terminal/File Manager lagi), tapi tambahkan **UI pengaturan SMTP di Site Settings** — terinspirasi dari pola Perfex CRM (`get_option()` dari database, bukan file config statis) yang sempat dicek sebagai referensi.

**Implementasi:**
- Migrasi `add_smtp_settings_to_site_settings_table`: kolom `mail_use_custom_smtp` (boolean), `mail_host`, `mail_port`, `mail_encryption` (`''`/`smtp`/`smtps` — persis sama dengan value `mail.mailers.smtp.scheme` Laravel, tidak perlu translasi), `mail_username`, `mail_password`, `mail_from_address`, `mail_from_name`.
- `SiteSetting::$casts['mail_password'] = 'encrypted'` — password SMTP terenkripsi otomatis pakai `APP_KEY` (dicek: baris mentah di DB benar-benar terenkripsi, decrypt-on-access lewat accessor model transparan).
- `SiteSetting::applyMailConfig()` — method baru, no-op kalau toggle `mail_use_custom_smtp` mati atau `mail_host` kosong (supaya default-nya tetap fallback ke `.env`), kalau aktif langsung `config([...])` menimpa `mail.default`/`mail.mailers.smtp.*`/`mail.from.*` saat runtime.
- Dipanggil dari `AppServiceProvider::boot()` (bukan di dalam `View::composer` yang di-defer sampai ada view dirender) — supaya berlaku juga di context console/queue worker, karena reminder/notifikasi partner sering justru dikirim dari situ, bukan dari request web biasa. Tetap dijaga `Schema::hasTable('site_settings')` seperti pola yang sudah ada di provider yang sama.
- UI: section baru "Pengaturan Email (SMTP)" di `ManageSiteSettings.php`, field-field mail_* cuma muncul kalau toggle-nya aktif (`->visible(fn ($get) => $get('mail_use_custom_smtp'))`). Field password pakai `->dehydrated(fn ($state) => filled($state))` supaya dikosongkan saat edit **tidak** menghapus password tersimpan, dan `mount()` sengaja `unset($data['mail_password'])` sebelum `$this->form->fill()` — password terdekripsi tidak pernah dikirim ke browser (kalau tidak, nilainya nongol di Livewire wire:snapshot walau input-nya tampil ter-mask).
- Header action baru **"Test Kirim Email"** — form kecil (alamat tujuan), langsung `applyMailConfig()` dari data TERSIMPAN lalu `Mail::raw(...)`, notifikasi sukses/gagal (gagal menampilkan `$e->getMessage()` mentah, persistent supaya tidak keburu hilang) — ini yang menggantikan seluruh proses manual `php artisan tinker` yang dipakai sepanjang debugging sesi ini, supaya ke depannya cukup klik dari panel, tidak perlu Terminal lagi.

Diverifikasi: 3 test baru (`SmtpSettingsPanelTest.php` — save & reload form termasuk cek password tidak ter-wipe saat field dikosongkan, test-email gagal ke host tidak valid, test-email sukses dengan `Mail::fake()`), full regression 223/224 (1 gagal pre-existing tidak terkait). Dicek manual lewat tinker: nilai di DB benar-benar terenkripsi (bukan plaintext), `applyMailConfig()` benar menimpa config saat toggle aktif dan benar-benar no-op (fallback ke `.env`) saat toggle mati.

## Fase 32 — Polish Panel Admin & Partner (Halaman, Favicon, Logo, Mobile App Settings) ✅ (selesai 2026-08-09)

Kumpulan perbaikan UI/UX kecil-menengah di panel Admin/Partner, diminta sekaligus dalam satu batch:

- **Toggle aktif/nonaktifkan halaman** — kolom `is_active` baru di `pages` table. Logic-nya dipusatkan di `Page::bySlug()` (satu-satunya titik yang dipanggil oleh ke-10 controller halaman inti: Home, About, Services, Portfolio, Blog, FAQ, Contact, Pricing, Team, Coming Soon) — `abort_if($page && ! $page->is_active, 404)` di situ saja, tidak perlu ubah 10 controller satu-satu. Halaman tanpa row `Page` sama sekali (belum pernah diisi admin) tetap render normal seperti sebelumnya — beda kondisi dari "sengaja dinonaktifkan".
  - **Bug serius ditemukan belakangan (2026-08-09, saat isi konten real)**: migrasi `add_is_active_to_pages_table` cuma andalkan `->default(true)` bawaan Laravel untuk backfill baris lama — di database dev, 9 dari 10 halaman (semua kecuali Home) ternyata jadi `is_active=0`, bukan `1` seperti seharusnya, bikin seluruh halaman situs 404 kecuali Home. Migrasi diperbaiki: tambah `DB::table('pages')->update(['is_active' => true])` eksplisit setelah `Schema::table()`, tidak lagi cuma andalkan default kolom. Data lokal sudah dikoreksi manual. **Penting**: karena migrasi ini belum pernah di-deploy ke hosting, kalau tidak ketahuan sekarang, situs production bisa 404 total begitu migrasi dijalankan di sana.
- **Sidebar admin terlalu renggang** — `gap-y-7` (28px) bawaan Filament antar nav group ditimpa jadi `0.5rem` lewat `panel-polish.blade.php`.
- **Settings App Mobile** — halaman admin baru, terpisah dari Site Settings (model+tabel sendiri, `MobileAppSetting`), isinya baru upload Logo Splash Screen. Sengaja dipisah dari `SiteSetting` supaya pengaturan web dan mobile app tidak saling campur ke depannya.
- **Favicon Admin/Partner ikut Site Settings** — `->favicon(fn () => ...)` di kedua panel provider, ambil dari `SiteSetting::current()->favicon`.
- **Logo di halaman login** — `->brandLogo()` (pakai `logo_dark`, untuk background terang) + `->darkModeBrandLogo()` (pakai `logo_light`) di kedua panel — otomatis tampil juga di sidebar, bukan cuma halaman login, karena keduanya pakai komponen `<x-filament::logo>` yang sama.
- **Redirect ke halaman login setelah reset password diminta** — sebelumnya halaman "Request Password Reset" Filament cuma clear form & tetap di situ setelah sukses kirim link. Bikin subclass `RequestPasswordReset` (Admin & Partner masing-masing, extends punya Filament) yang override `request()`: logic identik, cuma ganti `$this->form->fill()` jadi `$this->redirect(Filament::getLoginUrl())`. Notifikasi "link terkirim" tetap muncul di halaman login karena `Notification::send()` Filament pakai `session()->push()`, bukan flash sekali pakai untuk request yang sama.

Diverifikasi: 12 test baru (`PageActiveToggleTest`, `MobileAppSettingsPanelTest`, `PasswordResetRedirectTest`), full regression 232/233 (1 gagal pre-existing tidak terkait). Spot-check manual lewat curl: favicon & logo image benar-benar muncul di HTML halaman login (bukan cuma fallback teks), CSS sidebar baru ter-load.

## Fase 33 — Halaman Legal (Kebijakan Privasi, Syarat Ketentuan, Kontrak Kerja Sama) ✅ (selesai 2026-08-09)

Permintaan: fitur CRUD halaman legal di admin (create/edit/delete + meta title/description/keywords), mirip Post/Blog tapi tampilan beda karena untuk keperluan legal (bukan sekadar reskin blog).

**Keputusan desain:**
- Model baru `LegalPage` (bukan reuse `Page` yang sudah ada) — `Page` itu 10 slug tetap dengan `content` berupa `KeyValue` (field-per-field untuk isi section halaman statis seperti Home/About), bukan cocok untuk dokumen hukum panjang berbentuk prosa. `LegalPage` sebaliknya: jumlah row bebas (admin bisa tambah dokumen legal baru kapan saja), `content` satu `RichEditor` panjang, ada `slug` unik + `order` (urutan tampil) + `is_active` (pola sama seperti toggle Page di atas).
- Tampilan publik SENGAJA beda dari blog (`blog-modern.css`): tidak ada page-banner gradient animasi, tidak ada avatar/tombol share/sidebar "recent posts" ala blog — `legal-page.css` baru, header polos abu-abu muda, kolom baca sempit (max 760px), sidebar cuma daftar dokumen legal lain. Masih pakai warna aksen ungu situs (`#523fff`) supaya tetap terasa satu web, cuma registernya lebih tenang/formal.
- Route: `GET /legal` (index, daftar dokumen aktif) + `GET /legal/{legalPage:slug}` (show). `LegalPageController::show()` 404 kalau `is_active` false — pola sama persis dengan `Page::bySlug()` di Fase 32.
- Footer otomatis menampilkan link ke semua legal page aktif (tanpa perlu admin atur manual lewat menu) — ditambahkan ke composer global `AppServiceProvider` yang sudah ada (siteSettings/headerMenu/footerMenu/activePromotion), dengan guard `Schema::hasTable('legal_pages')` terpisah karena ini tabel baru yang bisa saja migrasinya belum jalan saat kode sudah ter-deploy.

**Susulan di hari yang sama** — permintaan tambahan: sebagian dokumen legal (kontrak kerja sama) perlu tampil sebagai dokumen resmi berkop surat (logo+alamat, nomor surat di bawah judul, footer formal), sementara yang lain (kebijakan privasi, syarat & ketentuan) tetap tampilan artikel biasa — plus bisa diekspor PDF.

- Kolom baru `type` (`policy`/`document`, default `policy`) + `document_number` (nomor surat, wajib diisi kalau `type=document`, divalidasi lewat `requiredIf` di form). `LegalPage::isFormalDocument()` jadi satu-satunya titik keputusan tampilan mana yang dipakai (dicek di `show.blade.php`, `pdf.blade.php`, dan tabel admin sekaligus) — bukan di-percabangkan berulang di banyak tempat.
- Kop surat (`legal-letterhead-*` class baru di `legal-page.css`): alamat kiri + logo kanan, garis tebal, judul center, nomor surat, garis tipis, lalu isi dokumen, ditutup footer formal berisi nama resmi perusahaan + alamat + disclaimer "diterbitkan secara elektronik". Halaman tipe `policy` tidak berubah sama sekali dari sebelumnya.
- **Export PDF** — user diberi pilihan (PDF asli server-side vs print-to-PDF browser), pilih **PDF asli** lewat `barryvdh/laravel-dompdf`. Route baru `GET /legal/{slug}/pdf`, view `legal/pdf.blade.php` **terpisah total** dari `legal/show.blade.php` — dompdf tidak mendukung flexbox/grid dengan baik, jadi versi PDF pakai layout table/block murni, bukan reuse CSS versi web. Logo dikonversi ke data URI base64 sebelum dikirim ke dompdf (bukan URL/path langsung) supaya tidak tergantung konfigurasi remote-fetch dompdf atau URL publik disk `media` bisa diakses balik oleh server sendiri.
- ~~**Belum ter-install**: `composer require barryvdh/laravel-dompdf` gagal dijalankan di environment ini~~ — **sudah ter-install** (lihat Fase 34, `repo.packagist.org` sempat down total di environment ini, pulih sendiri beberapa jam kemudian). PDF dicek langsung: 1 halaman asli, ukuran wajar (~950KB, mayoritas font DejaVu Sans yang di-embed dompdf).

Diverifikasi: 13 test baru total (`LegalPageTest.php`, semua hijau termasuk 2 test PDF), full regression bersih. Spot-check manual lewat curl: halaman `type=document` menampilkan seluruh elemen kop surat + tombol Download PDF, halaman `type=policy` sama sekali tidak menampilkan elemen kop surat (dikonfirmasi tidak ada class `legal-letterhead-*` sama sekali di HTML-nya) — data seed dihapus lagi setelah verifikasi.

## Fase 34 — Indexing Google/Bing Otomatis + Fix Bug URL Menu ✅ (selesai 2026-08-09)

**Bug ditemukan lewat laporan Google Search Console user** (bukan dari kode, dari data indexing asli): banyak halaman situs berstatus "Excluded by noindex tag"/"Discovered - currently not indexed". Diselidiki lewat kode: `ComingSoonMode` middleware, kalau toggle "Coming Soon" di Site Settings pernah aktif, menampilkan halaman Coming Soon (dengan `noindex`) di SEMUA URL kecuali beberapa yang dikecualikan — sementara `sitemap.xml` tetap menampilkan URL asli. Kalau Google sempat crawl saat mode itu aktif, persis menjelaskan pola datanya. Bukan bug kode yang perlu di-patch (perilakunya memang disengaja untuk mode Coming Soon), cuma perlu dipastikan togglenya mati + minta re-crawl manual lewat Search Console URL Inspection.

**Bug kode nyata yang ditemukan di jalan**: `DatabaseSeeder.php` bikin 24 baris `MenuItem` (Home/About/Services/dst di header+footer nav) pakai `route('about')` dkk **tanpa** parameter `absolute: false` — karena dijalankan dari CLI (bukan request browser), `route()` jatuh ke `APP_URL` di `.env` (`http://127.0.0.1:8000`) dan itu tersimpan permanen sebagai teks statis di kolom `url`. Beda dengan link Blog/Legal yang di-generate ulang tiap render langsung di Blade (otomatis ikut domain aktif). Kalau tidak ketahuan, deploy ke `nusatim.com` akan bikin **semua link menu navigasi** tetap mengarah ke localhost. Diperbaiki: seeder pakai `route(..., [], false)` (path relatif), 24 baris MenuItem + 1 Promotion yang sudah kadung absolut di database lokal dikonversi manual.

**Fitur baru — permintaan "biar artikel baru langsung terindeks":**
- Diriset dulu faktanya (bukan asumsi): Google Indexing API resmi cuma untuk `JobPosting`/`BroadcastEvent`, dipakai untuk artikel blog itu di luar cakupan resmi (ada yang tetap pakai secara tidak resmi, tidak ada laporan penalti, tapi tidak dijamin). Sitemap ping (`google.com/ping?sitemap=`) sudah **dimatikan Google sejak akhir 2023**, sekarang 404. User diberi pilihan lewat AskUserQuestion, pilih 2 dari 3: IndexNow (otomatis, resmi, Bing/Yandex saja bukan Google) + link Search Console siap pakai (semi-manual, khusus Google) — TIDAK pilih trik Indexing API area abu-abu.
- **IndexNow**: `App\Services\IndexNowService`, key acak 32-hex disimpan di `config('services.indexnow.key')`. Route `GET /{key}.txt` (constrained regex ke key yang dikonfigurasi, bukan wildcard sungguhan) untuk verifikasi kepemilikan domain. Hook otomatis di `Post::booted()` → `static::saved()`: submit URL kalau `$post->isLive()` (post baru publish ATAU post lama yang di-edit ulang, keduanya sinyal "tolong crawl ulang" yang valid) — sengaja **skip total saat `app()->runningUnitTests()`** supaya tidak ada request jaringan nyata yang ikut jalan di setiap test yang menyentuh model Post di seluruh test suite (baru ketahuan pas nulis test-nya sendiri: 1 hook di model bisa bikin lambat/hang test lain yang sama sekali tidak terkait fitur ini).
- **Link Search Console siap pakai**: field baru `search_console_resource_id` di Site Settings > Analytics (isi manual dari address bar Search Console, formatnya beda-beda tergantung tipe property: `sc-domain:` atau URL prefix — tidak bisa ditebak otomatis). Tombol "Cek di Search Console" di halaman edit Post (cuma muncul kalau post live), buka tab baru ke `search.google.com/search-console/inspect?resource_id=...&id=...` — tetap perlu 1 klik manual "Request Indexing" di sana (Google tidak expose itu lewat API publik untuk konten biasa), tapi tidak perlu copy-paste URL lagi.
- **Dependency**: `guzzlehttp/guzzle` ternyata belum pernah ter-install di proyek ini sama sekali (fitur `Http::` facade baru pertama kali dipakai lewat IndexNowService ini) — baru ketahuan lewat error "Class GuzzleHttp\HandlerStack not found" saat testing. Sudah di-`composer require`.

**Catatan sampingan, di luar scope, sengaja tidak dikerjakan sekarang**: `composer audit` nemu CVE aktif di `laravel/framework` versi yang dipakai sekarang (CRLF injection di validasi email default, versi terpasang < 13.10.0 kena). Tidak di-patch sekarang karena di luar permintaan user, dan upgrade Laravel patch version di branch downgrade PHP 8.1 ini perlu dicek dulu kompatibilitasnya - perlu keputusan eksplisit user, bukan diputuskan sepihak.

Diverifikasi: 8 test baru (`IndexNowTest` 5, `SearchConsoleDeepLinkTest` 3), full regression 253/254 (1 gagal pre-existing tidak terkait). Dicek manual: file verifikasi IndexNow benar-benar serve via curl, PDF Legal Pages (Fase 33) juga langsung dites ulang begitu dompdf ter-install dan lolos.
