...
- **Belum ter-install**: `composer require barryvdh/laravel-dompdf` gagal dijalankan di environment ini — `repo.packagist.org` timeout total (situs lain normal), kemungkinan pembatasan jaringan spesifik di sandbox ini, bukan masalah proyek. Kode PDF (controller + route + view) sudah lengkap dan siap pakai, tinggal user jalankan composer install itu sendiri di terminal lokal/hosting mereka.

Diverifikasi: 13 test baru total (`LegalPageTest.php` — 11 jalan hijau + 2 test PDF sengaja `markTestSkipped()` selama package belum ter-install, bukan gagal), full regression 243/246 (1 gagal pre-existing tidak terkait + 2 skip PDF yang disengaja). Spot-check manual lewat curl: halaman `type=document` menampilkan seluruh elemen kop surat + tombol Download PDF, halaman `type=policy` sama sekali tidak menampilkan elemen kop surat (dikonfirmasi tidak ada class `legal-letterhead-*` sama sekali di HTML-nya) — data seed dihapus lagi setelah verifikasi.
