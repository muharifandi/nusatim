...
<nav
    ...
    role="navigation"
    {{--
        Upstream uses grid-cols-[1fr_auto_1fr], which stretches the overview
        text and the per-page selector into separate 1fr tracks - the
        selector ends up dead-center in the bar instead of next to the
        overview text. auto_auto_1fr keeps the first two compact and left,
        letting only the pagination links claim the remaining (1fr) space
        on the right.
    --}}
    {{
        $attributes->class([
            'fi-pagination grid grid-cols-[auto_auto_1fr] items-center gap-x-3',
            'fi-simple' => $isSimple,
        ])
    }}
>
